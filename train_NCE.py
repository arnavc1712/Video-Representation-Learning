...
from model.model_NCE import Model
...
def train(loader, model, optimizer, opt):
    ...
    for epoch in range(opt['epochs']):
        ...
        for (vis_feats, pos_lang_feats, neg_lang_feats) in loader:
            ...
            pos_nce_prob, neg_nce_prob = model(vis_feats, pos_language_feat, neg_language_feat)

            # Stack pos-neg scores and BCE scores
            stacked_nce_prob = torch.cat((pos_nce_prob, neg_nce_prob.view(-1, 1)), 0)
            nce_labels = generate_bce_labels(pos_nce_prob.shape[0], neg_nce_prob.shape[0]*neg_nce_prob.shape[1])

            # NCE scores, note that negative sample num = (batch_size / gpu_num) * neg_per_sample
            loss = torch.nn.functional.binary_cross_entropy_with_logits(stacked_nce_prob, nce_labels, reduction='sum')
            ...
            step += 1
            print('(epoch %d), loss = %.6f, current lr = %.5E, pos-neg-score'
                  % (epoch, print_loss, optimizer.param_groups[0]['lr']), pos_nce_prob.mean(), neg_nce_prob.mean())
